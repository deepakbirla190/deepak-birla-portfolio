...
const Navbar = () => {
  ...
  return (
    <header className={`${isVisible ? '!sticky' : '!sticky'} top-0 z-50`}>
          ...
          {isVisible ? (
            ...
            <Link href="/">
              <div className="animate-fade-up text-primary-content relative flex items-center gap-3 transition-all duration-300 md:static">
                <Logo />
                <span className="text-primary-content">deepak_birla</span>
              </div>
            </Link>
          )}
          ...
          <ul
            ...
            {navItems.map(({ label, href }) => (
              <li
                ...
                className="border-border flex items-center border-b px-4 text-xl lg:text-2xl md:border-y-0 md:border-e md:text-base md:first:border-s md:last:ml-auto md:last:border-none md:last:px-0 lg:px-8">
                <Link
                  href={href}
